...
class AuthenticationController(val accountsDao: AuthenticationDao)
  extends Controller
    with LoginLogout
    with AuthConfigImpl
{
  ...
  def authenticate(username: String, password: String): Future[User] = {
    def checkPassword(account: User): Boolean = {
      import com.github.t3hnar.bcrypt._
      password.isBcrypted(account.password)
    }

    for {
      optUser <- accountsDao.findUserByLogin(username)
  ...
  def authenticateRequest = Action.async { implicit request =>
    def proccessForm(data: (String, String)) = {
      val result = data match { case (username, password) =>
          for {
            account <- authenticate(username, password)
            result <- gotoLoginSucceeded(account.login)
          } yield result
      }
      result.fallbackTo{
  ...
  def registerNewUser(inputs: (String, String, String)): Future[Result] = inputs match {
    case (username, password, retyped) =>
      ...
      val result = for {
        ...
        account = User(available.get, username, password.bcrypt, User.Role.normal)
        newAccount <- accountsDao.createAccount(account)
      } yield Ok(views.html.login("created", "Account created! Log in!"))

      result.fallbackTo{
        Future.successful(
          BadRequest(views.html.login("register", "This username is not available!"))
        )
      }
  }
  ...
  def registerRequest = Action.async { implicit request =>
    registrationForm.bindFromRequest.fold(
      formWithErrors =>
        Future.successful(BadRequest(views.html.login("register", formWithErrors.globalError.get.message))),
