...
import search.core.SearchCriteriaSerializationTest.{apply => _, _}
...
object DeserializerTest extends TestSuite {
  ...
  val tests = this {
    "Test [1] - Deserializer single" - {
      ...
      assert(res == Ok(token, "", 1))
    }

    "Test [2] - Deserializer single with transformer" - {
      val d = single('9')(_.toString.toInt)
      val res = d.deserialize("9")
      assert(res == Ok(9, emptyInput, 1))
    }

    "Test [3] - Deserializer check" - {
      val d = check((c: Char) => c.isDigit)(identity)

      * - {assert(d.deserialize("1") == Ok('1', emptyInput, 1))}
    ...
    "Test [4] - Deserializer isExhausted" - {
      ...
      * - {assert(d.deserialize(emptyInput[Char]) == Ok(true, emptyInput[Char], 0))}
    ...
    "Test [5] - Deserializer foldWhile" - {
      ...
      * - {assert(d.deserialize("123d") == Ok(Seq('1', '2', '3'), "d", 3))}
      * - {assert(d.deserialize(emptyInput[Char]) == Ok(Seq[Char](), emptyInput[Char], 0))}
      * - {assert(d.deserialize("123") == Ok(Seq('1', '2', '3'), emptyInput[Char], 3))}
    }

    "Test [6] - Deserializer zeroOrMore" - {
      val d = zeroOrMore[Char, Int]('!', seq => seq.size)

      * - {assert(d.deserialize("no") == Ok(0, "no", 0))}
      * - {assert(d.deserialize(emptyInput[Char]) == Ok(0, emptyInput[Char], 0))}
      * - {assert(d.deserialize("!!!!!!") == Ok(6, emptyInput[Char], 6))}
      * - {assert(d.deserialize("d!!!!!!") == Ok(0, "d!!!!!!", 0))}
      * - {assert(d.deserialize("!!!!!!sss!!!!") == Ok(6, "sss!!!!", 6))}
    }

    "Test [7] - Deserializer oneOrMore" - {
      val d = oneOrMore[Char, Int]('!', seq => seq.size)

      * - {assert(d.deserialize("!d") == Ok(1, "d", 1))}
    ...
    "Test [8] - Deserializer times" - {
      ...
      * - {assert(d.deserialize("!!") == Ok('!', emptyInput[Char], 2))}
    ...
    "Test [9] - Deserializer oneOf" - {
      ...
      * - {assert(d.deserialize("12") == Ok('1', "2", 1))}
    ...
    "Test [10] - Deserializer andThen" - {
      ...
      * - {assert(d3.deserialize("12") == Ok("12", emptyInput[Char], 2))}
      * - {assert(d3.deserialize("12d") == Ok("12", "d", 2))}
    ...
    "Test [11] - Deserializer end" - {
      ...
      * - {assert(d.deserialize("a") == Ok('a', emptyInput[Char], 1))}
